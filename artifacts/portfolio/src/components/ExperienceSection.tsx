      ...
      <div className="container mx-auto px-8 md:px-16 lg:px-24 max-w-5xl">
        ...
        >
          <motion.h2 
            ...
            className="text-4xl md:text-5xl font-serif font-bold text-[#1A1A1A] mb-16 text-center md:text-left"
          >
            Where I've Worked
          </motion.h2>

          <div className="relative border-l-2 border-primary ml-3 md:ml-4 space-y-12">
            {experiences.map((exp, index) => (
              <motion.div key={index} variants={itemVariants} className="relative pl-8 md:pl-10">
                {/* Dot */}
                ...
                <div className="flex flex-col mb-4">
                  ...
                  <span className="italic text-[#6B6B6B] mt-1">
                    {exp.company}
                  </span>
                </div>
                ...
                <ul className="space-y-3">
                  {exp.bullets.map((bullet, bIndex) => (
                    <li key={bIndex} className="flex items-start text-[#1A1A1A] font-sans leading-relaxed">
                      <span className="text-primary font-bold mr-3 mt-[2px]">→</span>
                      <span>{bullet}</span>
                    </li>
                  ))}
                </ul>
